/*
  ...
  (c) 2014 by kenshin54
  adapted 2015 eric.ridley.johnson@gmail.com
*/
  ...
  var toggleBox = function(event) {
    ...
    var selection = $.popline.utils.selection().text();
    if (selection.length > 0 && !$.popline.current.keepSilentWhenBlankSelected()) {
      var target= $.popline.current.target, bar = $.popline.current.bar;
      if (bar.is(":hidden") || bar.is(":animated")) {
        bar.stop(true, true);
        var pos = Position().mouseup(event);
        
        $.each($.popline.buttons, function (title, button) { 
        	if (button.dynamicText && button.dynamicText.map) {
        		var displayText = button.dynamicText.map(selection);
        		if (displayText) {
					$(".popline-" + title + "-button span a span").html(displayText); 
				}
				else {
					$(".popline-" + title + "-button span a span").html(button.dynamicText.defaultText);
				}
        	}
        });
      ...
      init: function() {
        ...
        var makeButtons = function(parent, buttons) {
          for (var name in buttons) {
            ...
            if (button.dynamicText || button.staticText) {
              $button.children(".popline-btn").append("<a href='javascript:void(0);'><span class='popline-text popline-background-fix'>" + (button.dynamicText ? button.dynamicText.defaultText : button.staticText) + "</span></a>");
            }
